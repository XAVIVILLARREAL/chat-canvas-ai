...
1. **Etapa 0** — 0.1 schema+migraciones → 0.2 event_stream → 0.3 secretos BYOK → 0.4 sandbox → 0.5 OpenAPI → 0.6 i18n infra
 2. **Chat** — A.0 proyectos scope → A.1 appshell → A.2 persistencia+**papelera** → A.3 provider+BYOK → A.4 UX chat → **A.4b cola de streaming** → A.5 medidor → A.6 config → A.7 encargo+**forecast** → A.8 resume → A.9 ramas → **A.10 comparador A/B** → **A.12 presencia/reacciones** → **A.0b import ChatGPT** → **A.11 export PDF/MD**
 3. **Runtime** — C.1 Reasonix → C.2 router/telemetría → **C.3 robustez+circuit breaker** → **H.9a sandbox** → **C.8 tools web** → **C.9 visión** → **C.10 browser automation** → **C.11 hooks de ciclo de vida** → C.5 caché → C.6 Ollama → C.7 registro proveedores
...
7. **Memoria** — D.1 ledger → D.2 knowledge+lock → D.3 memory rail → D.4 gobernanza → D.5 índice semántico → D.6 router+shards → D.7 blame-rung → D.8 memorias multi-tipo
 8. **Skills** — G.1 CRUD+**perfiles BYOK (G.1b)** → G.2 editor+tool-gating → G.3 compilador+**import rules (G.3b)** → G.4 laboratorio → G.6 rutinas → **G.7 identidad viva (avatares/bio)**
 ...
 10. **Editor avanzado** — B.6 menciones → B.7 hunks → B.8 consola→agente → B.9 versiones → **B.10 LSP integration**
 11. **Kanban** — KR.1 tablero → KR.2 bloques animados → KR.4 evidencia → KR.5 filtros/salud (KR.3 tras N.7)
 12. **Oficina** — F.0 design system → F.1 fundaciones ReactFlow → F.2 nodos vivos → F.4 tareas/kanban sobre canva → F.7 ⌘K → F.6 perf 60fps
 13. **Cierre MVP-2** — E.2/E.3 re-corridos → **GATE MVP-2** (PRD F7-F11 + micro-mejoras)
...
13. **Automatización** — F.3 edges → F.5 identidad IA → (resto del Etapa 6 completo): deploy-spec, compiler, multi-runtime, conectores
 14. **Nube 24/7** — N.1-N.6 orquestación → **N.5b dashboard personal** → **N.7 modo nube** → **N.8 puentes (WA/TG/Discord/Slack)** → **N.9 Agente Supervisor** + **S.1/S.2 despliegue** + AUTH nube + RLS
 15. **Sync** — L.1 SyncHub → L.2 cliente sync → L.4 push dispatcher
 16. **GitHub** — M.1 auth/repos → M.2 ciclo git → M.3 PRs/issues
 17. **Marketplace** — O.1 bundles → O.2 MCP público → **O.4 share público** → **O.3 release v1.0**
