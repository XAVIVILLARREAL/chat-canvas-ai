...
SDD ✓ (si es feature) → fila en MATRIZ ✓ → tests unit/integration verdes → E2E humano (móvil+desktop, es+en) con video en `evidence/` → `pnpm check:all`+`cargo test` → CHANGELOG+ESTADO actualizados → commit semántico.
