...
| Fase | Nombre | Pruebas |
|---|---|---|
| A.0 | Proyectos como SCOPE (FUNDACIÓN) ✅ 2026-08-27 | Unit: repos filtran por project_id ✅; scopes global→proyecto ✅ (`GLOBAL_PROJECT_ID`, `setting_resolve`; `settings_scopes_override_no_muta_global`). Integration: cross-proyecto vacío ✅; override local no muta global ✅ (core+HTTP); tabs restauran tras reinicio ✅ (`ui.tabs`/`ui.active_project`, test HTTP con reinicio real). E2E: 2 proyectos alternando ✅ (humana 6/6 pasos: crear único→alternar→persistir→**entrar por CARD-grid**→default; skill global vs copia local ✅ backend (`/share`+`/copy`, HTTP test) — UI E2E con Skills Lab ⬜). HUMANA @core: cambiar de proyecto, nada se mezcla ✅. UI: ProjectSwitcher + ProjectsGrid modal (fail-open). Nota host: gateway en CANVAS_AI_PORT=3031 (zombie supervisado en 3030) |
| A.1 | AppShell + stores ✅ 2026-08-27 | Vitest stores+hook ✅ (chat-ui-store 3/3: vista inmutable por immer, sesión activa). E2E: layout móvil 375 (BottomNav alterna Canvas/Chat ✅ — header compacto ≤900px sin overflow 412px) y desktop 1440 (sidebar tab Sesiones + panel chat + memory rail placeholder ✅ — humana 5/5 pasos: crear sesión, enviar mensaje, **conversación restaura tras recarga**, volver a Canvas). Server: /api/sessions CRUD + /messages (roundtrip tras reinicio ✅). Chat sin proveedor: hint A.3, sin datos inventados |
| A.2 | Persistencia SQLite (settings CIFRADA) ✅ 2026-08-27 | Cargo test repositorios ✅ (desde 0.1/A.1). Integration: roundtrip mensaje con project_id ✅. **Settings cifradas (T.SEC)**: `vault::set/reveal/delete_secret_setting` — fila settings guarda SOLO `{__secret: key_ref}` (safe-by-default en `settings_resolved`); endpoints `PUT /api/settings/:id/secret` + `POST …/reveal`; test `settings_cifradas_dump_sin_plaintext`: GET sin plaintext ✅ · reveal roundtrip ✅ · **dump sqlite sin secreto** ✅ · sin KEK → fail-closed sin filtrar ✅ · override local por proyecto ✅. Keychain OS deferred a shell Tauri |
| A.3 | Trait AgentProvider + BYOK + DeepSeekDirect ✅ 2026-08-27 | Unit con mock-server SSE ✅ (`sse_chunks_en_orden`: 5 chunks en orden + DONE; `no_streaming_respuesta_completa`) · integration orden de chunks ✅ · **REAL OpenRouter :free** (free-first, $0 — opt-in CANVAS_REAL_LLM=1, fallback across free pool, 429 upstream → SKIP honesto) ✅ · registro universal: un `OpenAICompatProvider` parametrizado cubre openai/openrouter/deepseek/ollama (default_base_url por tipo) · key desde vault (jamás en claro) · `POST /api/sessions/:id/chat`: persiste user+assistant + usage rollup, 400 honesto sin provider ✅ (chat_con_provider_mock) · frontend /chat con fallback local · tool-calls→Reasonix en Etapa C |
| A.4 | UX del chat (perillas, tool-calls, diff, slash) 🟡 núcleo ✅ 2026-08-27 | **Streaming SSE en vivo** ✅: server `/chat/stream` (deltas→done→persist) + frontend reader con parse SSE + cursor en vivo — test `chat_stream_sse_deltas_y_persistencia` (deltas en orden + persisted:true + 2 mensajes) y humana paso 6 (provider mock SSE → "Hola streaming A4 vivo") · **slash `/help`** ✅ (notice UI; otros slash → aviso honesto "llega en etapa posterior") · **costo/usage** ✅ (tokens por mensaje assistant + rollup sesión) · resiliencia: resolve_providers itera habilitados (providers caídos no bloquean) ✅ · markdown vivo ⬜ (requiere decidir renderer dep) · perillas modelo/temp ✅ backend (request opcional; UI perillas en A.6) · diff aprobable ⬜ (con tool-calls, Etapa C) |
