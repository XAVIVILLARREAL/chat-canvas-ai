...
> Generada automáticamente desde los planes (fuente de verdad = los .md). Regenerar tras cada cambio de fases.

> Capas de prueba por fase: **[U]**nit vitest · **[I]**ntegración cargo/mock · **[E]**2E funcional Playwright · **[H]**umana suite Playwright modo persona. Toda fase GUI exige E+E; toda fase de gate exige H.
...
| A.0 | Proyectos como TENANTS: cards + tabs + historial aislado (FUNDACIÓN, va sobre todo lo demás) | - Unit: repos filtran por project_id en TODAS las consultas; resolución de scopes global→local
  - Integration: proyecto A no ve NADA de B (queries cross-tenant vacías); override local no muta el global; restauración de  |
...
| B.6 | @menciones de archivos/símbolos (ganadora torneo #002) | ver sección |
| B.7 | Revisión por HUNKS (ganadora torneo #124) | ver sección |
| B.8 | Consola preview → "enviar error al agente" (ganadora torneo #143) | ver sección |
...
## Etapa P · Centro MCP (transversal tras 2) — `plan-p-centro-mcp.md`
...
| C.6 | OllamaProvider local, ready-to-plug (SDD-006 §5) | Unit: detección/parsing de /api/tags y errores de conexión. Integration con Ollama real: chat streaming completo + embeddings generados; cambio de preset KV verificado por memoria consumida. E2E humano: conectar Ollama d |
| C.7 | Registro universal de proveedores API (análisis exhaustivo OpenCode/models.dev → [SDD-007](../SDD-007-analisis-opencode-modelsdev.md)) | ver sección |

## Etapa 4 · Memoria V3Code — `plan-d-memoria-v3code.md`

| Fase | Nombre | Pruebas |
|---|---|---|
| D.0 | Restaurar artefactos de referencia (prerequisito) | ver sección |
...
| D.8 | Motor de memorias multi-tipo configurable (SDD-006 §3) | Unit decay/scoring con fixtures. Integration: contradicción invalida (bi-temporal) sin borrar; consolidación idempotente. E2E humano: ajustar λ de un tipo y ver el ranking cambiar; leakage test entre proyectos |

## Etapa 5 · Cierre Base (tag v0.1) — `plan-e-integracion-total.md`

| Fase | Nombre | Pruebas |
|---|---|---|
| — | (fases en formato tabla original — ver archivo) | |
...
## Etapa 8 · Motor de pruebas (orden: H.1→H.2→H.5→H.6→H.3→H.4→H.7→H.8→H.9) — `plan-h-motor-pruebas.md`
...
| H.9 | Computadora persistente del agente (patrón Grok Bot, local-first) | Cargo test drivers tras trait común. Integration: container crea archivo → reinicia sesión → archivo sigue ahí; snapshot→restore exacto. Chaos: matar container → recrear desde snapshot. E2E humano: abre terminal del agen |
...
## Etapa 12 · Sync multi-device + Co-Work (+L.4 push) — `plan-l-sync-cowork.md`
...
| Fase | Nombre | Pruebas |
|---|---|---|
| U.11 | Primitivas dopaminérgicas + JUICE calibrado (en [F·F.0](./plan-f-canva-oficina.md#f0)) | ver sección |
| U.22 | Micro-feedback en ejecución (cada segundo cuenta) | ver sección |
| U.33 | Celebraciones escalonadas, gated y con cofres funcionales | ver sección |
| U.44 | Progresión honesta: niveles, rachas-perdonables y heatmap | ver sección |
| U.55 | Flow-protection y anti-spinner (el hallazgo más importante) | ver sección |
| U.66 | Glanceables fuera de la app (widget de 2 DATOS, patrón Duolingo) | ver sección |
| U.77 | Onboarding emocional: el unboxing (Arc/Raycast) | ver sección |
| U.88 | Anti-dark-patterns (vigilado por [U.9]) | ver sección |

## Transversal S · Despliegue/Costos/Stack — `plan-s-despliegue-costos.md`

| Fase | Nombre | Pruebas |
|---|---|---|
| — | ver archivo (formato especial) | |
