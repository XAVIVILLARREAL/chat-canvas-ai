...
Infra 100% verde + sistema spec-driven con suite humano 12/12 (SDD-002). **ADR-005 aceptado:** modelo de despliegue dual (local-first + servidor nube) con refactor a workspace Cargo como prerrequisito de Etapa 1. **Multiplataforma cerrado (SDD-005):** proyecto Android nativo versionado en `src-tauri/gen/android/` (toolchain instalada en servidor: JDK 21 + SDK 34 + NDK r27), CI con matriz desktop ubuntu/windows/macos en cada push, workflow manual "Android Build" para APK debug, y `docs/MULTIPLATAFORMA.md` con comandos por plataforma (iOS requiere Mac para `ios init`). Siguiente paso: mini-SDD técnico PLAN A y Fase A.1.
