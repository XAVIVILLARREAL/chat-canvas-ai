...
- **Etapa 1 EN CURSO** (A.0–A.4 ✅ → A.5) — [plan-a-chat-codex](./SDDs/SDD-001-plan-base/plan-a-chat-codex.md):
  - 🟡 **A.0 Proyectos como SCOPE (fundación) — backend+switcher COMPLETOS 2026-08-27**: endpoints `/api/projects` (CRUD, global protegido) + `/api/settings/{project_id}` (override local / herencia global / clear) + `GLOBAL_PROJECT_ID` reservado · core: `ensure_global_project`, `setting_resolve`, `settings_resolved`, `project_setting_clear` · frontend: `ProjectSwitcher` (fail-open sin gateway, persiste `ui.active_project`, crea proyectos) + `src/lib/projectsApi.ts` (VITE_API_BASE '/api' normalizado) · gateway puerto configurable `CANVAS_AI_PORT` (default 3030; el host tiene el zombie empresa-dev-server supervisado en 3030 → usar 3031) · vite proxy configurable `CANVAS_GATEWAY` · tests: core override-no-muta-global ✅ · server HTTP override/cross-proyecto/tabs-restauran-tras-reinicio ✅ · humana @core 5/5 pasos (crear/alternar/persistir/nada se mezcla) ✅ · i18n 3 keys nuevas ×12 locales ✅. Pendiente A.0: card-grid de proyectos, skill global vs copia local.
...
### A.1 AppShell + stores ✅ (2026-08-27, commit en curso)
- Server: `/api/sessions` CRUD + `/api/sessions/:id/messages` (roundtrip tras reinicio verificado por HTTP).
- Frontend: `ChatPanel` (mensajes + input + memory rail placeholder Etapa D), `SessionsList` (tab Sesiones del sidebar), `chat-ui-store` (Zustand+immer: vista canvas↔chat, sesión activa), `useSessions` hooks (React Query + invalidación), BottomNav móvil (≤900px) + header compacto (toggles ocultos en móvil, overflow 412px eliminado), QueryClientProvider global.
- Chat sin datos inventados: enviar persiste el mensaje del usuario; hint "proveedores BYOK en A.3".
- Tests: vitest 3/3 store · humana desktop 5/5 (crear sesión → enviar → **restaura tras recarga**) · móvil 3/3 (BottomNav) · regresión móvil-drawer + responsive ✅.

### A.2 Settings cifradas ✅ (2026-08-27)
- `core::vault`: `set_secret_setting` / `secret_setting_ref` / `reveal_secret_setting` / `delete_secret_setting` — fila settings = `{__secret: key_ref}` (safe-by-default), secreto en vault_entries (AES-256-GCM).
- Server: `PUT /api/settings/:id/secret` + `POST /api/settings/:id/reveal` (server-side, THREAT-MODEL). OpenAPI actualizado.
- Gate: GET settings sin plaintext ✅ · reveal roundtrip ✅ · **dump sqlite sin secreto** ✅ · sin KEK fail-closed ✅ · override local ✅. 31/31 rust.

### A.3 AgentProvider + BYOK ✅ (2026-08-27)
- `core::providers`: trait `AgentProvider` (name/capabilities/send_message/send_message_stream/cancel) + `OpenAICompatProvider` (cubre openai/openrouter/deepseek/ollama — todos dialecto OpenAI) + parse SSE + `default_base_url` por tipo.
- Server: `resolve_provider` (primer provider enabled del proyecto, key desde vault) + `POST /api/sessions/:id/chat` (persiste user → LLM → persiste assistant con usage rollup; 400 honesto sin provider).
- Frontend: enviar en ChatPanel usa /chat si hay provider (respuesta real), fallback local si no.
- Tests: mock SSE orden de chunks ✅ · no-streaming ✅ · chat endpoint con mock+vault ✅ · REAL OpenRouter :free (opt-in, fallback multi-modelo, 429→SKIP) ✅. 36/36 rust.
- Nota: pool free de OpenRouter se rate-limita upstream (429 transitorio) — el fallback multi-modelo es el comportamiento real de dev; el .env traía modelo stale ('stealth/ox-alpha' ya no existe).

### A.4 UX del chat — núcleo ✅ (2026-08-27)
- **Streaming SSE**: `POST /api/sessions/:id/chat/stream` (persiste user → deltas → persist assistant → meta final con usage) + frontend reader (fetch stream, parse SSE, cursor en vivo).
- **Slash commands honestos**: /help con notice; otros → "llega en etapa posterior" (sin fake).
- **Costo/usage**: tokens por assistant + rollup sesión. **Resiliencia**: `resolve_providers` itera habilitados — un provider caído no bloquea el chat.
- Tests: `chat_stream_sse_deltas_y_persistencia` ✅ · humana 8/8 (streaming con provider mock SSE Node) ✅. Pendiente A.4+: markdown vivo (decisión de renderer), diff aprobable (Etapa C con tool-calls), perillas UI (A.6).
