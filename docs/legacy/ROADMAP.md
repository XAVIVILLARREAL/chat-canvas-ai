...
> **ESTADO (2026-08-03):** Implementado y EN VIVO en `https://empresa-dev.xtremediagnostics.com` (túnel Cloudflare → `localhost:7688` en `pve`). Los 3 pilares funcionan: mapa de sesiones en el canva, voz (Edge TTS + STT navegador), y evidencia por prompt (log de herramientas + screenshots).

- [x] Canva libre (React Flow): cajas, notas, flechas, colores, zoom, touch.
- [x] Insertar **ventanita de agente** → abre sesión real (click/touch).
- [x] Ventana chat estilo terminal: streaming + logs de tools + código coloreado.
- [x] Motor del agente: **opencode** (MIT) servidor headless (`opencode serve`, puerto 7699).
- [x] **Voz:** hablar con la IA sin abrir la ventanita (🎤/🔊 en el canva) — Edge TTS en servidor + Web Speech API del navegador.
- [x] **Evidencia por prompt:** el proxy Hono captura los eventos de opencode (text/tool/screenshot) y los muestra en el chat (log de herramientas expandible + tarjetas de imagen).
- [x] Varias ventanitas = varias sesiones simultáneas (cada ventanita crea su propia sesión opencode).
- [ ] La sesión sigue aunque cierres la ventanita (mini-estado en el canva) — parcial: la sesión persiste en el server, falta el mini-estado en vivo.
- [x] Persistencia del canva (posiciones, esquemas) en localStorage (SQLite llega con sesiones).
- [ ] Auth básica + mobile responsive (PWA).
- [ ] Múltiples directorios reales por ventanita (hoy todas en `/opt/empresa-desarrollo-autonoma`).

**Despliegue actual:**
| Servicio | Estado |
|---|---|
| `empresa-app.service` (Hono + web, 7688) | ✅ active |
| `empresa-opencode.service` (opencode serve, 7699) | ✅ active |
| Túnel Cloudflare `empresa-desarrollo-autonoma-tunnel` | ✅ healthy |
